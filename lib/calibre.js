...
let calibre = (recipe_name) => {
  ...
  let date = new Date();
  let date_file = `${date.getFullYear()}_${date.getMonth()}_${date.getDate()}`;
  let date_str = `${date.getFullYear()}/${date.getMonth()}/${date.getDate()}`;

  winston.debug("Config:", config);
  let output_file =
    `${out_path}/${recipe_name}_${date_file}.mobi`;
  ...
  if (config.smtp && config.smtp.server && config.email) {
    exec(
      `${bin_dir}/${calibre_smtp} --attachment "${output_file}" --relay "${config.smtp.server}" --port "${config.smtp.port}" --username "${config.smtp.username}" --password "${config.smtp.passwd}"  --encryption-method TLS --subject "${subject_prefix} (${date_str})" "${config.smtp.from}" "${config.email}"  "(${date_str})"`
    );
  }
